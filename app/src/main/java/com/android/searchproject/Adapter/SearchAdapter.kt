...
class SearchAdapter(val mItems:MutableList<DocumentsItem>,val context: Context):RecyclerView.Adapter<SearchAdapter.Holder>() {
        ...
        holder.imageView.setOnClickListener {  //클릭이벤트추가부분
            ...
            if(holder.heart.isInvisible) {
                ...
                (context as MainActivity)
                    .addLikeItem(mItems[position].thumbnail_url.toString(),
                ...
                (context as MainActivity)
                    .deleteLikeItem(mItems[position].thumbnail_url.toString(),
                        mItems[position].display_sitename.toString(),
                        mItems[position].datetime.toString())
            }
        }
...
//        holder.date.text = mItems[position].datetime
        ...
        val old_format = ZonedDateTime.parse(date, formatter)
        val new_format = old_format.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))
        holder.date.text = new_format
